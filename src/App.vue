    ...
    <template v-if="hasGame">
        ...
        >
          ...
          <div class="min-w-0">
            <div class="truncate text-sm font-black tracking-tight">
              {{ gameStore.selectedClub?.shortName }}
            </div>
            <div
          ...
          <template v-for="(item, index) in navItems" :key="item.to ?? `divider-${index}`">
            ...
            >
              ...
              <svg
                ...
                stroke-width="1.8"
              >
                <path d="M7 7h13m-4-4 4 4-4 4M17 17H4m4 4-4-4 4-4" />
              </svg>
              <svg
      ...
      <div class="min-h-screen md:pl-[228px]">
        <header class="sticky top-0 z-30 border-b border-slate-200/90 bg-white/90 backdrop-blur-xl">
          <div
            class="flex min-h-[86px] items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8"
          >
            <RouterLink to="/dashboard" class="flex min-w-0 items-center gap-3 md:gap-4">
              <ClubBadge
                v-if="gameStore.selectedClub"
                :club="gameStore.selectedClub"
                class="md:hidden"
              />
              <div class="min-w-0">
          ...
          <nav class="flex gap-1 overflow-x-auto border-t border-slate-100 px-3 py-2 md:hidden">
            ...
            <button class="rounded-lg px-3 py-2 text-xs font-bold text-rose-600" @click="reset">
              Новая игра
            </button>
          </nav>
        </header>

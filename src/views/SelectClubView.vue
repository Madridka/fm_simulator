...
const divisions = [
  { label: 'Все дивизионы', value: 'all' },
  { label: divisionNames[1], value: 1 },
  { label: divisionNames[2], value: 2 },
  { label: divisionNames[3], value: 3 },
  { label: divisionNames[4], value: 4 },
]
...
const difficulty = (club: Club): { label: string; className: string } => {
  ...
  if (strengthScore >= 115) {
    return { label: 'Легко', className: 'bg-emerald-100 text-emerald-800' }
  }
  if (strengthScore >= 82) {
    return { label: 'Нормально', className: 'bg-amber-100 text-amber-800' }
  }
  return { label: 'Сложно', className: 'bg-rose-100 text-rose-800' }
}
  ...
  <section class="space-y-5">
    <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
      <div>
        ...
        <p class="mt-1 text-sm text-slate-600">Россия, четыре дивизиона, 40 клубов.</p>
      </div>
      ...
      <article v-for="club in filteredClubs" :key="club.id" class="rounded-lg border border-white/70 bg-white/90 p-4 shadow-[0_18px_50px_rgba(20,46,38,0.1)]">
        <div class="flex items-start justify-between gap-3">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="font-semibold text-slate-950">{{ club.name }}</h2>
